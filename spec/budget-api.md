...
`ready_to_assign` **(005; global — changes/026 reverts 025's month-scoping):** `SUM(Transaction.amount WHERE is_income = true, joined through the user's accounts) − SUM(BudgetAllocation.allocated_amount)` — all-time on both sides, one figure that is identical on every `month`. Assigning money into a future month debits this pool immediately, so the same dollars can never appear assignable in two months. Only the per-category `available` / `rollover` values are month-bounded. Was, pre-005: "uncategorized inflow minus total allocated"; 005 switched the income side to the explicit `is_income` flag; 025 briefly made both sides month-scoped and 026 reverted that (it let the same income look assignable in the current month and a future month at once). `is_income` and `category_id` are mutually exclusive (`spec/transactions.md`), so this is equivalent to the old uncategorized-inflow query restricted to the new explicit flag. No per-transaction backfill of existing data — a one-time per-account "Starting Balance" `is_income` transaction (see `spec/transactions.md`'s Notes) reconciles current bank balances into this sum instead, so `ready_to_assign` is not silently `0` on day one.
...
- 021 (2026-08-27) — envelope-style credit-card budgeting. `Category.payment_account_id`
...
- 026 (2026-08-31) — reverted 025's month-scoping of `ready_to_assign`. It is
  global again (all-time income − all-time allocations), one figure on every
  month, so assigning into a future month debits the pool immediately and the
  same dollars can't look assignable in two months. Per-category `available` /
  `rollover` stay month-bounded (unchanged from 025). `budget_api.py`
  `get_budget` only; `tests/test_budget_api.py`: the two 025 month-scoped-RTA
